...
public:
	...
	virtual void	Invoke(Args * args) {
		...
		{
			BSAudioManager__GetSoundHandle(*g_BSAudioManager, &menu->playingSound, &((BGSSoundDescriptorForm*)ff)->soundDescriptor, 0, 0x10, 0);
		...
		}
		{
			struct SetPerkPoints_int_Code : Xbyak::CodeGenerator {
				SetPerkPoints_int_Code(void * buf) : Xbyak::CodeGenerator(4096, buf)
				{
					...
					L(retnLabel);
					dq(SetPerkPoints_int.GetUIntPtr() + 6);
				}
			};
