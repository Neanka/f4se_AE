 ...
 * _offsets
 *
 * 0x8C TESCellAttachDetachEvent
 * 0xA0 TESCellFullyLoadedEvent
 *
 *
 * 0x64 TESActorLocationChangeEvent
 */
...
public:
    ...
    BSSoundHandle playingSound; // 180
};

STATIC_ASSERT(sizeof(LevelUpMenu) == 0x188);

typedef UInt32 (*_LevelupMenuProcessMessage)(LevelUpMenu* menu, UIMessage* message);
// 0x0B38360 // 48 89 5C 24 08 48 89 74 24 10 57 48 83 EC 40 48 8B F1 8B 4A 10 
...
extern RVA<_LevelupMenuPlayPerkSound> LevelupMenuPlayPerkSound;
// 0x0B39750 // 48 89 5C 24 08 57 48 83 EC 30 48 8B F9 8B CA 33 DB E8 ? ? ? ? 48 85 C0 74 ? 80 78 1A 5F 75 ? 48 8B 98 88 00 00 00 48 81 C7 80 01 00 00 
*/

typedef UInt8 (*_BSSoundHandle__Stop)(BSSoundHandle* unk1); // unk1 is LevelUpMenu+0x180
extern RVA<_BSSoundHandle__Stop> BSSoundHandle__Stop;
// 0x01AC7B70 // 40 53 48 83 EC 20 8B 19 83 FB FF 74 ? C6 41 05 02
...
typedef void (*_BSAudioManager__GetSoundHandle)(void* param1, BSSoundHandle* param2, void* param3, void** param4, int param5,
                                            void** param6);
extern RVA<_BSAudioManager__GetSoundHandle> BSAudioManager__GetSoundHandle;
//48 8B C4 48 89 58 08 48 89 68 10 48 89 70 18 57 41 54 41 55 41 56 41 57 48 81 EC A0 00 00 00 44 8B 0D ? ? ? ?

typedef void (*_BSSoundHandle__Play)(BSSoundHandle* param1);
extern RVA<_BSSoundHandle__Play> BSSoundHandle__Play; //40 53 48 83 EC 20 8B 19 83 FB FF 74 ? C6 41 05 01

extern RVA<void*> g_BSAudioManager;
...
public:
    ...
    UInt32 pad8C;
    void* unk90[31];
    TESWorldSpace* parentWorldSpace; // 188
    void* unk190[40];
...
void RVA_InitFavorites();

typedef UInt64 (*_FavoritesManager_useQuickkey)(FavoritesManager* favmanager, SInt32 qkindex);
extern RVA<_FavoritesManager_useQuickkey> FavoritesManager_useQuickkey;

typedef UInt64 (*_FavoritesManager_getQuickKeyIndex)(FavoritesManager* favmanager, BSFixedString* str);
extern RVA<_FavoritesManager_getQuickKeyIndex> FavoritesManager_getQuickKeyIndex;
...
extern RVA<_RegisterMenuOpenCloseEvent>	RegisterMenuOpenCloseEvent;
...
extern RVA<_UnregisterMenuOpenCloseEvent> UnregisterMenuOpenCloseEvent;

void RVA_InitInventory();

typedef UInt32 (*_EquipItem_int)(void* param1, void* param2, void* param3, SInt8 param4, SInt8 param5, void* param6,
                                 SInt8 param7, bool forceequip, SInt8 param9, SInt8 param10, bool preventunequip);
extern RVA<_EquipItem_int> EquipItem_int;

typedef UInt8 (*_UnEquipItem_int)(void* unkmanager, void* actor, void* unkstruct, SInt8 param4, void* equipslot,
                                  SInt8 param6, SInt8 param7, bool abPreventEquip, SInt8 param9, SInt8 param10,
                                  void* param11);
...
extern RVA<_getInventoryItemByHandleID> getInventoryItemByHandleID;

extern RVA<ItemMenuDataManager*> g_itemMenuDataMgr;
extern RVA<void*> unk_itemManager;

void RVA_InitSpells();

typedef void (*_Spell_Cast)(VirtualMachine* vm, UInt32 StackID, SpellItem* spell, TESObjectREFR* source, TESObjectREFR* target);
extern RVA<_Spell_Cast> Spell_Cast;

typedef void (*_Actor_DispellSpell)(VirtualMachine* vm, UInt32 StackID, Actor* source, SpellItem* spell);
extern RVA<_Actor_DispellSpell> Actor_DispellSpell;

typedef void (*_ImageSpaceModifier_Apply)(VirtualMachine* vm, UInt32 StackID, TESImageSpaceModifier* imad, float strength);
extern RVA<_ImageSpaceModifier_Apply> ImageSpaceModifier_Apply;

typedef void (*_ImageSpaceModifier_Remove)(VirtualMachine* vm, UInt32 StackID, TESImageSpaceModifier* imad);
extern RVA<_ImageSpaceModifier_Remove> ImageSpaceModifier_Remove;

void RVA_InitCells();

typedef TESWorldSpace*(*_WorldCameraMode__DetermineMapWorld)();
extern RVA<_WorldCameraMode__DetermineMapWorld> WorldCameraMode__DetermineMapWorld;

typedef const char*(*_TESWorldSpace__GetWorldMapTextureName)(TESWorldSpace* worldspace);
extern RVA<_TESWorldSpace__GetWorldMapTextureName> TESWorldSpace__GetWorldMapTextureName;

void RVA_InitMap();

struct HUDMarkerData__GetFlashSymbolNameForMapMarker_struct
{
    void* unk0;
    void* unk8;
    void* unk10;
...
typedef BSFixedString*(*_HUDMarkerData__GetFlashSymbolNameForMapMarker)(HUDMarkerData__GetFlashSymbolNameForMapMarker_struct str);
extern RVA<_HUDMarkerData__GetFlashSymbolNameForMapMarker> HUDMarkerData__GetFlashSymbolNameForMapMarker;

void RVA_InitAddresses();

//void RVA_InitUIManager();

void RVA_InitCWAddresses();

void RVA_InitFavoritesMenuExAddresses();

class FavoritesMenu : public GameMenuBase
{    
};

typedef UInt32 (*_FavoritesMenuProcessMessage)(FavoritesMenu* menu, UIMessage* message);
extern RVA<_FavoritesMenuProcessMessage> FavoritesMenuProcessMessage;

typedef bool (*_FavoritesManager__inputEventUser__ShouldHandleEvent)(BSInputEventUser* inputEventUser, InputEvent* inputEvent);
extern RVA<_FavoritesManager__inputEventUser__ShouldHandleEvent> FavoritesManager__inputEventUser__ShouldHandleEvent;

void RVA_InitVATS();

extern RVA<void*> g_VATS;
...
public:
    ...
    tHashSet<PipboyTableItem, UInt32>    table;  // 30
    tArray<void*> unk60;
...
public:
    ...
    PipboyObject* inventoryObject; // E8 // 440 from datamanager start
    ...
    tHashSet<HSF0Entry, UInt32> hsF0; // something related to sorting? // 448
    ...
    tHashSet<HS120Entry, BGSComponent*> hs120; // 478
    tArray<PipboyObject*> inventoryObjects; // 150 // 4A8
    tHashSet<UInt32> hs168; // contain FormTypes for update? // 4C0
};
...
{
public:
    BSTEventSink<PlayerCharacterQuestEvent::Event> es1;
    BSTEventSink<TESQuestEvent::Event> es2;
    void* unkA8;
    PipboyArray* unkPA_B0;
    void* unkB8;
    void* unkC0;
    void* unkC8;
    void* unkD0;
...
public:
    ...
    PipboyObject* mapDataObject; // always same as 0x90 // E8
    /*
    object with fields:
    CurrCell (string)
    ...
    World (object)
        ...
        Quests (array of objects)
            ...
            Name
            Shared
            QuestId (array of uints FormIDs)
            X
    ...
    */
    ...
    tHashSet<MarkerInfo, TESFullName*> markersInfo; // F0
    ...
    tHashSet<marker, UInt32> markers; // 120
    ...
    PipboyObject* worldPlayerPosition; // object with fields X (float) Y (float) and Rotation (float)
    PipboyObject* localPlayerPosition; // object with fields X (float) Y (float) and Rotation (float)
    PipboyObject* worldCustomMarkerPosition;
    // probably custom marker. object with fields X (float) Y (float) Visible (bool) and Height (uchar)
    PipboyObject* localCustomMarkerPosition;
    // probably custom marker. object with fields X (float) Y (float) Visible (bool) and Height (uchar)
    PipboyObject* worldPAPosition;
    // probably PA marker. object with fields X (float) Y (float) Visible (bool) and Height (uchar)
    PipboyObject* loaclPAPosition;
    // probably PA marker. object with fields X (float) Y (float) Visible (bool) and Height (uchar)
    PipboyObject* worldMapExtents; // object with fields SWX(float) NEY(float) NWX(float) NEX(float) NWY(float) SWY(float)
    PipboyObject* localMapExtents; // object with fields SWX(float) NEY(float) NWX(float) NEX(float) NWY(float) SWY(float)
};
...
public:
    ...
    void* unk328;
    UInt32 unk330;
    // init'd as ctor 2nd param. 2 for PowerArmorModMenu, 1 for CookingMenu, 0 for RobotModMenu and ExamineMenu
...
STATIC_ASSERT(sizeof(WmkActiveEffect) == 0xB0);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_vtable) == 0x00);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_item) == 0x48);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_elapsed) == 0x78);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_duration) == 0x7C);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_magnitude) == 0x80);
STATIC_ASSERT(offsetof(WmkActiveEffect, m_flags) == 0x84);

// In F4SE 0.6.7 (1.10.82) the GetActiveEffects() methods is not defined.
class WmkMagicTarget
{
public:
	virtual ~WmkMagicTarget();
	...
	virtual tArray<WmkActiveEffect*>* GetActiveEffects();
...
struct HUDSubtitleDisplayEvent
{
public:
    BSFixedString SpeakerName;
    BSFixedString SubtitleText;
};

// 0x230
class HUDSubtitleText : public HUDComponentBase
{
public:
    BSGFxShaderFXTarget SpeakerName_tf; // F8
    BSGFxShaderFXTarget SubtitleText_tf; // 1A8
    UInt64 valueEventSink__HUDSubtitleDisplayEvent; // 258
    HUDSubtitleDisplayEvent ev; // 260
    bool hUDSubtitleDisplayEventReceived; // 270
    SimpleLock hUDSubtitleDisplayEventdataLock; // 278
    UInt64 unk280[10];
};

STATIC_ASSERT(sizeof(HUDSubtitleText) == 0x2D0);
