...
namespace FindBestWeapons
{
	...
	{
		NiPointer<TESObjectREFR> ref = NULL;

		const UInt32* handle = commandTargetCompanionRef;
	...
	void FindBestWeapons(TESObjectREFR* ref, bool checkammo)
	{
		...
		ref->inventoryList->inventoryLock.UnlockRead();
	...
	void UnEquipCurrentWeapon(Actor* actor)
	{
		...
		actor->inventoryList->inventoryLock.UnlockRead();
	}
}